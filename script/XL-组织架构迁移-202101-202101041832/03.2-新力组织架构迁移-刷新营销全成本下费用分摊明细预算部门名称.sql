...
insert into #zz_ChangeDepartment
select c.BUGUID,c.BUName,a.SpecialUnitGUID,a.SpecialUnitName,a.Year,
CONVERT(VARCHAR(4), a.Year) + '年-' + c.BUName + '-'+ a.SpecialUnitName
from ys_SpecialBusinessUnit a 
join ys_SpecialBusinessUnit b on a.BUGUID = b.BUGUID and b.SpecialUnitGUID = a.ParentGUID and b.SpecialUnitName = '营销全成本'
join myBusinessUnit c on a.BUGUID = c.BUGUID and c.IsCompany = 1 and c.BUName = '湾区城市公司'

if exists(select 1 from #zz_ChangeDepartment)
    begin
        begin tran
            begin
            ---------------------------------------------------------------------------------------------- 申请单 BEGIN----------------------------------------------------------------------------------------------
            ...
            -- 申请单分摊明细
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment  a
                join fy_Apply_FtDetail b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName
            -- 申请单月分摊明细
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment         a
                join fy_Apply_FtDetail_Period b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName
            ...
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment            a
                join fy_Contract_FtDetail_Period b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName
                join cb_Contract c on b.ContractGUID = c.ContractGUID and c.IsFyControl = 1

            -- 部门科目合同使用明细
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment           a
                join ys_DeptCost2ContractUseDtl b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName
                join cb_Contract c on b.ContractGUID = c.ContractGUID and c.IsFyControl = 1
            -- 合同结算表 组织架构
            ...
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment             a
                join fy_HTBalance_FtDetail_Period b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName
            ...
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment             a
                join cb_DeptCostUseDtl b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName
                join cb_Contract c on b.ContractGUID = c.ContractGUID and c.IsFyControl = 1

            -- 合同变更明细
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment             a
                join fy_HtAlter_FtDetail_Period b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName
            ...
            */
            ...
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment             a
                join cb_Expense_FluentDetails b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName
            ...
            */
            ...
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment a
                join cb_Loan_FtDetail b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName
            ...
            */
            ...
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment  a
                join ys_DeptCostUseDtl b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName

            -- 分摊费项实际使用审核信息
            update b set b.DeptName = a.TargetDeptName
            from #zz_ChangeDepartment         a
                join ys_DeptCostUsedInfoForSH b on b.DeptGUID = a.DeptGUID and b.DeptName <> a.TargetDeptName
